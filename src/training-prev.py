    ...
    def objective(trial):
        ...
        if verbose:
            logger.log(
                ...
                also_print=False,   # Optuna's own INFO logging already prints to stdout; avoid duplicate console noise
            )
        return mean_score
...
def run_hyperparameter_search(X_train_prebalance, y_train_prebalance, model_type,
    ...
    logger.log(
        ...
        "Optuna prints each trial's result to stdout as it completes (not silent)."
    )
